...
class PluginsController extends SystemAppController {
    ...
    public function uninstall($plugin = null) {
        if (!in_array($plugin, $this->Plugin->DENY_LIST) && is_dir($this->Plugin->PLUGIN_FOLDER . $plugin)):
            ...
            try {
                ...
                $this->Session->setFlash("'" . $plugin . "' " . __d("system","was uninstalled successfully"));
            } catch (Exception $e) {
                $this->Session->setFlash(__d("system","Error at disable plugin") . " '" . $plugin . "', " . __d("system","the error was the following") . ": " . $e->getMessage());
            }

        else:
            $this->Session->setFlash(__d("system","You cannot disable") . " '" . $plugin . "'");
        endif;
    ...
    public function reset($plugin = null) {
        if (!in_array($plugin, $this->Plugin->DENY_LIST) && is_dir($this->Plugin->PLUGIN_FOLDER . $plugin)):
            try {
                ...
                $this->Session->setFlash(__d("system","Error trying to reset the plugin named") . ": '" . $plugin . "'");
            }
        else:
            $this->Session->setFlash(__d("system","You cannot disable") . " '" . $plugin . "'");
        endif;
    ...
    public function enable($plugin = null) {
        if (!in_array($plugin, $this->Plugin->DENY_LIST) && is_dir($this->Plugin->PLUGIN_FOLDER . $plugin)):
            try {
                ...
                $this->Session->setFlash(__d("system","Error at enable plugin") . " '" . $plugin . "', " . __d("system","the error was the following") . ": " . $e->getMessage());
            }
        else:
            $this->Session->setFlash(__d("system","You cannot enable") . " '" . $plugin . "'");
        endif;
        $this->redirect($this->referer());
    }
    ...
    public function delete($plugin = null) {
        if (!in_array($plugin, $this->Plugin->DENY_LIST) && is_dir($this->Plugin->PLUGIN_FOLDER . $plugin)):
            ...
            CakeLog::write('activity', AuthComponent::user('username') . __d("system","deleted the plugin") . " '" . $plugin . "'");
            $this->Session->setFlash(__d("system","Plugin named") . " '" . $plugin . "' " . __d("system","deleted successfully."));
        else:
            $this->Session->setFlash(__d("system","You cannot delete") . " '" . $plugin . "'");
        endif;
        $this->redirect($this->referer());
    }
        ...
        foreach($officialRepos as $key => $value):
            if($value["name"] == $plugin):
                ...
                if($x == true):
                    ...
                    $this->Session->setFlash(__d("system","Cannot install ") . Inflector::humanize(substr($value["name"], strpos($value["name"], "-")+1)) . " > " . $x);
                endif;
