<div class="col-md-4">
    <div class="bs-example widget">
        ...
        <div id="quickAddContent" class="highlight collapse">
            ...
            <div class="form-group <?= $this->Form->isFieldError('User.role') ? 'has-error has-feedback' : ''; ?>">
                ...
                <div class="col-sm-9">
                    <?php
                    foreach (array_flip(Configure::read("SystemRole")) as $key => $value):
                        if (AuthComponent::user()['role'] < $key):
                            Configure::delete("SystemRole." . $value);
                        endif;
                    endforeach;
                    echo $this->Form->input('role', array(
                        'options' => array_map(function($val) {
                            return __d("system", $val);
                        }, array_flip(Configure::read("SystemRole"))),
                        'class' => 'form-control',
                        'id' => 'inputRole'
                    ));
                    ?>
    ...
    <div class="highlight">
        <?= $this->HTML->link(sprintf(__d("system", "List %s"), __d("system", "Users")), array('plugin' => 'system', 'controller' => 'users', 'action' => 'index'), array('class' => 'btn btn-default')); ?>
        <?= $this->HTML->link(sprintf(__d("system", "Manage %s"), __d("system", "Roles")), array('plugin' => 'system', 'controller' => 'users', 'action' => 'roles'), array('class' => 'btn btn-default')); ?>
        <?= $this->HTML->link(sprintf(__d("system", "Add %s"), __d("system", "Users")), array('plugin' => 'system', 'controller' => 'users', 'action' => 'add'), array('class' => 'btn btn-default')); ?>
    </div>
</div>
<div class="col-md-12">
    <div class="bs-example widget">
    ...
    <div class="highlight">
<?= $this->HTML->link(sprintf(__d("system", "Clean Activity Log")), array('plugin' => 'system', 'controller' => 'config', 'action' => 'cleanlog'), array('class' => 'btn btn-default')); ?>
    </div>
</div>